...
namespace Docnet.Core.Editors
{
  internal class DocEditor : IDocEditor
  {
    ...
    public byte[] DeleteAttachment(byte[] file)
    {
      lock (DocLib.Lock)
      {
        using (var docWrapper = new DocumentWrapper(file, null))
        {
          return DeleteAttachment(docWrapper);
        }
      }
    }

    public byte[] DeleteAttachment(string file)
    {
      lock (DocLib.Lock)
      {
        using (var docWrapper = new DocumentWrapper(file, null))
    ...
    private static byte[] DeleteAttachment(DocumentWrapper docWrapper)
    {
      using (var stream = new MemoryStream())
      {
        int count = fpdf_view.FPDFDoc_GetAttachmentCount(docWrapper.Instance);
        for (int i = count - 1; i >= 0; i--)
        {
          var success = fpdf_ppo.FPDFDoc_DeleteAttachment(docWrapper.Instance, i);

          if (!success)
          {
            throw new DocnetException("failed to delete attachments files");
          }
        }
