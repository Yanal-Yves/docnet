...
namespace Docnet.Tests.Integration
{
  ...
  public sealed class DocReaderTests
  {
    ...
    [Theory]
    [InlineData(Input.FromFile, "Docs/simple_0.pdf", null, 0)]
    [InlineData(Input.FromBytes, "Docs/simple_0.pdf", null, 0)]
    [InlineData(Input.FromFile, "Docs/pdf_with_attachment.pdf", null, 1)]
    [InlineData(Input.FromBytes, "Docs/pdf_with_attachment.pdf", null, 1)]
    public void GetAttachmentCount_WhenCalled_ShouldReturnCorrectResults(Input type, string filePath, string password, int expectedCount)
    {
      using (var reader = _fixture.GetDocReader(type, filePath, password, 10, 10))
      {
        var count = reader.GetAttachmentCount();
